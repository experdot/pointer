...
function processDeepSeekData(
  data: DeepSeekChat[],
  folderId?: string
): { chats: Chat[]; successCount: number; errorCount: number; skippedCount: number } {
  const chats: Chat[] = []
  let successCount = 0
  let errorCount = 0
  let skippedCount = 0
  ...
  return { chats, successCount, errorCount, skippedCount }
}
...
export function parseExternalChatHistory(jsonContent: string): ParseResult {
  try {
    ...
    const chats: SelectableChatItem[] = []
    ...
    return {
      ...
      formatType,
      chats,
      message: `找到 ${chats.length} 个可导入的聊天记录`
    }
  } catch (error) {
 ...
 * 导入选中的聊天记录
 */
export function importSelectedChats(selectedItems: SelectableChatItem[], folderName?: string): ImportResult {
...
export function importExternalChatHistory(jsonContent: string): ImportResult {
  try {
    ...
    // 限制为50条记录
    const limitedChats = parseResult.chats.slice(0, MAX_IMPORT_LIMIT)
    const skippedCount = Math.max(0, parseResult.chats.length - MAX_IMPORT_LIMIT)

    // 生成文件夹名称
    const folderName = generateFolderName(parseResult.formatType, parseResult.chats[0]?.originalData)
