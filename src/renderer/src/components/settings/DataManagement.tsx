...
import { Card, Button, Space, message, Modal, Upload, Typography, Divider, Popconfirm, App, Table, Checkbox, Input, Tag } from 'antd'
import {
  DownloadOutlined,
  UploadOutlined,
  DeleteOutlined,
...
import { importExternalChatHistory, parseExternalChatHistory, importSelectedChats, SelectableChatItem } from '../../utils/externalChatImporter'
import { ChatFolder } from '../../types'

const { Text, Paragraph } = Typography

export default function DataManagement() {
  const { settings, resetSettings, importSettings, exportSettings } = useSettings()
    ...
    reader.onload = (e) => {
      try {
        ...
        if (result.success) {
          ...
          const currentChats = currentState?.chats || []
          ...
          // 合并新聊天到现有聊天中
          const mergedChats = [...currentChats, ...result.chats]
    ...
    reader.onload = (e) => {
      try {
        ...
        if (parseResult.success) {
          setSelectableChatItems(parseResult.chats)
          setSelectedChatIds([])
          setCustomFolderName('')
    ...
    <Card size="small" title="数据管理" bordered={false}>
        ...
        <div style={{ marginBottom: '16px' }}>
          ...
          <Space direction="horizontal" size="small" style={{ marginBottom: '4px' }}>
            ...
            <Upload
              accept=".json"
              showUploadList={false}
              beforeUpload={handleSelectiveImport}
            >
              <Button 
                icon={<SelectOutlined />} 
                type="primary"
      ...
      >
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          {/* 文件夹名称输入 */}
          <div>
            <Text strong style={{ marginBottom: '8px', display: 'block' }}>导入文件夹名称（可选）</Text>
            <Input
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Space>
              <Checkbox
                indeterminate={selectedChatIds.length > 0 && selectedChatIds.length < selectableChatItems.length}
