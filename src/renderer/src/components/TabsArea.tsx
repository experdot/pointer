...
import { ChatWindow, ChatWindowRef } from './chat/index'
...
export default function TabsArea() {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (ctrlKey && !altKey && !shiftKey && key === 'p') {
        event.preventDefault()
        const chat = state.chats.find((c) => c.id === activeTabId)
  ...
  }, [state.activeTabId, state.chats, dispatch])
    ...
    .map((chatId) => {
      ...
      return {
        ...
        children: (
          <ChatWindow 
            chatId={chatId} 
            ref={(ref) => setChatWindowRef(chatId, ref)}
          />
        ),
