...
export default function ChatHistoryTree({ onChatClick }: ChatHistoryTreeProps) {
  ...
  const handleDrop = useCallback(
    (info: DropInfo) => {
      ...
      if (!dropToGap && dropNodeInfo.type === 'folder') {
        ...
        if (dragNodeInfo.type === 'folder') {
          ...
          const folderChats = state.chats.filter((chat) => chat.folderId === dropNodeInfo.id)
          const allOrders = [
      ...
      if (dropToGap) {
        ...
        const dragItem =
          dragNodeInfo.type === 'chat'
            ? state.chats.find((chat) => chat.id === dragNodeInfo.id)
            : state.folders.find((folder) => folder.id === dragNodeInfo.id)

        const dropItem =
          dropNodeInfo.type === 'chat'
            ? state.chats.find((chat) => chat.id === dropNodeInfo.id)
    ...
    [parseNodeKey, state.folders, state.chats, dispatch]
  )
