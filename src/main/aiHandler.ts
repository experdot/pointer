...
import { LLMConfig, ChatMessage } from '../shared/types'
...
class AIHandler {
  ...
  private setupHandlers() {
    ...
    ipcMain.handle('ai:send-message-streaming', (event, request: AIRequest) => this.sendMessageStreaming(event, request))
  ...
  private async sendMessageStreaming(event: Electron.IpcMainInvokeEvent, request: AIRequest): Promise<void> {
    try {
      // 为当前请求创建新的AbortController
      const abortController = new AbortController()
      this.abortControllers.set(request.requestId, abortController)
      
      const response = await fetch(`${request.config.apiHost}/chat/completions`, {
        method: 'POST',
        headers: {
  ...
  private async sendMessage(request: AIRequest): Promise<{ success: boolean; content?: string; reasoning_content?: string; error?: string }> {
    try {
      const response = await fetch(`${request.config.apiHost}/chat/completions`, {
        method: 'POST',
        headers: {
  ...
  private async testConnection(config: LLMConfig): Promise<{ success: boolean; error?: string }> {
    try {
      const response = await fetch(`${config.apiHost}/models`, {
        method: 'GET',
        headers: {
